...
//		System.out.println("测试"+key);
	...
	public void move(long gametime) {
		int currentSpeed = isSpeedBoosted ? boostedSpeed : originalSpeed;
	...
	public void add(long gameTime) {
	    ...
	    if (this.isRapidFire) {
	        ...
	        for (String dir : directions) {
	            ...
	            element.setPlayerId(this.playerId);
...
//		传递一个固定格式 {x:3,y:5,f:up} json格式
    ...
    private int originalSpeed = 3; // 原始速度
    private int boostedSpeed = 10;  // 加速后的速度

    public void setSpeedBoost(boolean speedBoosted) {
        this.isSpeedBoosted = speedBoosted;
    }
    ...
    private final long SHOOT_INTERVAL = 300; // 正常射击间隔（毫秒）
    private final long RAPID_FIRE_INTERVAL = 100; // 加速射击间隔（毫秒）

    public void setRapidFire(boolean rapidFire) {
        this.isRapidFire = rapidFire;
    }
 ...
 // 在 Play.java 中新增方法
    public long getRemainingTime(long startTime, long activeTime) {
        long elapsedTime = System.currentTimeMillis() - startTime;
        return Math.max(0, activeTime - elapsedTime); // 返回剩余时间（毫秒）
    }
 ...
 // 在 Play.java 中新增属性
    ...
    // 添加方法设置效果开始时间
    public void setInvincibleStartTime(long time) {
        this.invincibleStartTime = time;
    }

    public void setSpeedBoostStartTime(long time) {
        this.speedBoostStartTime = time;
    }

    public void setRapidFireStartTime(long time) {
        this.rapidFireStartTime = time;
    }

    // 添加方法获取剩余时间
    public long getInvincibleRemainingTime() {
        long currentTime = System.currentTimeMillis();
        return Math.max(0, 5000 - (currentTime - invincibleStartTime));
    }

    public long getSpeedBoostRemainingTime() {
        long currentTime = System.currentTimeMillis();
        return Math.max(0, 5000 - (currentTime - speedBoostStartTime));
    }

    public long getRapidFireRemainingTime() {
        long currentTime = System.currentTimeMillis();
        return Math.max(0, 5000 - (currentTime - rapidFireStartTime));
    }
